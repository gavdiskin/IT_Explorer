import { useState, useMemo } from 'react';
import { Link, navigate } from '../router';
import Icon from '../icons';
import { PLACES, CATEGORIES, FOOD_FILTERS } from '../data';
import { PlaceCard, Slot, PriceMark, StarRating } from '../shared';
import { GMap } from '../gmap';

export function MapPage({ tweaks, city, route, savedSet, onToggleSave }) {
  const layout = tweaks.mapLayout || 'floating';
  if (layout === 'split')  return <MapSplit  tweaks={tweaks} city={city} route={route} savedSet={savedSet} onToggleSave={onToggleSave}/>;
  if (layout === 'drawer') return <MapDrawer tweaks={tweaks} city={city} route={route} savedSet={savedSet} onToggleSave={onToggleSave}/>;
  return <MapFloating tweaks={tweaks} city={city} route={route} savedSet={savedSet} onToggleSave={onToggleSave}/>;
}

function useMapState(initialQuery, initialCategory, city, tweaks) {
  const [query, setQuery] = useState(initialQuery || '');
  const [activeCat, setCat] = useState(initialCategory || 'all');
  const [foodTags, setFoodTags] = useState([]);
  const [selectedId, setSel] = useState(null);
  const [openFilters, setOpenFilters] = useState(false);

  const all = PLACES.filter(p => (!p.optional || tweaks.showCannabis) && p.city === city);

  const filtered = useMemo(() => {
    let r = all;
    if (activeCat !== 'all') r = r.filter(p => p.category === activeCat);
    if (foodTags.length) r = r.filter(p => foodTags.some(t => (p.cuisine || []).includes(t)));
    if (query.trim()) {
      const q = query.toLowerCase();
      r = r.filter(p => [
        p.name, p.subcategory, p.area, p.category, ...(p.tags || []), ...(p.cuisine || []),
        (CATEGORIES.find(c => c.id === p.category) || {}).label || ''
      ].join(' ').toLowerCase().includes(q));
    }
    return r;
  }, [all, activeCat, foodTags, query]);

  return { query, setQuery, activeCat, setCat, foodTags, setFoodTags, filtered, selectedId, setSel, openFilters, setOpenFilters };
}

function MapSplit({ tweaks, city, route, savedSet, onToggleSave }) {
  const st = useMapState(route.query?.q, route.query?.cat, city, tweaks);

  return (
    <main className="map-page route-mount">
      <div className="map-split">
        <aside className="map-split__list">
          <ListHeader st={st} count={st.filtered.length} city={city}/>
          <CategoryStrip st={st} tweaks={tweaks}/>
          {st.foodTags.length > 0 && <ActiveFoodTags st={st}/>}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 12 }}>
            {st.filtered.map(p => <PlaceCard key={p.id} place={p} compact savedSet={savedSet} onToggleSave={onToggleSave}/>)}
            {st.filtered.length === 0 && <EmptyState/>}
          </div>
        </aside>
        <div style={{ position: 'relative' }}>
          <GMap pins={st.filtered} selectedId={st.selectedId} onSelect={p => st.setSel(p.id)} city={city}/>
          <SelectedPopup st={st} savedSet={savedSet} onToggleSave={onToggleSave}/>
          <FabColumn/>
        </div>
        <MobileSheet st={st} savedSet={savedSet} onToggleSave={onToggleSave}/>
      </div>
    </main>
  );
}

function MapFloating({ tweaks, city, route, savedSet, onToggleSave }) {
  const st = useMapState(route.query?.q, route.query?.cat, city, tweaks);

  return (
    <main className="map-page route-mount">
      <GMap pins={st.filtered} selectedId={st.selectedId} onSelect={p => st.setSel(p.id)} city={city}/>

      <div className="map-filterbar">
        <Icon.search size={16}/>
        <input value={st.query} onChange={e => st.setQuery(e.target.value)} placeholder='Search places, cuisines, areas…'/>
        {st.query && <button className="btn btn-sq btn-ghost" onClick={() => st.setQuery('')}><Icon.x size={14}/></button>}
        <button className="btn btn-sq btn-ghost" onClick={() => st.setOpenFilters(true)} aria-label="Filters"><Icon.sliders size={16}/></button>
      </div>

      <div style={{ position: 'absolute', top: 76, left: 14, right: 14, zIndex: 5, display: 'flex', gap: 6, overflowX: 'auto' }} className="hide-scrollbar">
        <CategoryStrip st={st} tweaks={tweaks}/>
      </div>

      <SelectedPopup st={st} savedSet={savedSet} onToggleSave={onToggleSave}/>
      <FabColumn/>

      <div className="only-tablet-up" style={{ position: 'absolute', left: 14, bottom: 24, zIndex: 5, background: 'var(--bg-card)', borderRadius: 14, padding: 12, boxShadow: 'var(--shadow)', minWidth: 240, maxHeight: 'calc(100vh - 200px)', display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <div className="h4" style={{ fontSize: 14 }}>{st.filtered.length} {st.filtered.length === 1 ? 'place' : 'places'}</div>
          <button className="btn btn-ghost" style={{ padding: 4, fontSize: 11 }} onClick={() => st.setOpenFilters(true)}><Icon.sliders size={14}/> Filter</button>
        </div>
        <div style={{ overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 360 }}>
          {st.filtered.slice(0, 8).map(p => <PlaceCard key={p.id} place={p} compact savedSet={savedSet} onToggleSave={onToggleSave}/>)}
        </div>
      </div>

      <MobileSheet st={st} savedSet={savedSet} onToggleSave={onToggleSave}/>
      {st.openFilters && <FilterModal st={st} tweaks={tweaks} onClose={() => st.setOpenFilters(false)}/>}
    </main>
  );
}

function MapDrawer({ tweaks, city, route, savedSet, onToggleSave }) {
  const st = useMapState(route.query?.q, route.query?.cat, city, tweaks);
  const [drawerOpen, setDrawerOpen] = useState(true);

  return (
    <main className="map-page route-mount">
      <GMap pins={st.filtered} selectedId={st.selectedId} onSelect={p => st.setSel(p.id)} city={city}/>

      <button onClick={() => setDrawerOpen(o => !o)} className="map-drawer-toggle btn"
              style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow)' }} aria-label="Toggle list">
        <Icon.menu size={16}/> {drawerOpen ? 'Hide' : 'Show'} list
      </button>

      <FabColumn/>

      <aside className={'map-drawer' + (drawerOpen ? ' is-open' : '')}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '0 0 12px' }}>
          <Icon.search size={16}/>
          <input value={st.query} onChange={e => st.setQuery(e.target.value)} placeholder='Search…' style={{ flex: 1, border: 0, outline: 'none', font: 'inherit', fontSize: 14, background: 'transparent' }}/>
          {st.query && <button className="btn btn-sq btn-ghost" onClick={() => st.setQuery('')}><Icon.x size={14}/></button>}
          <button className="btn btn-sq btn-ghost" onClick={() => st.setOpenFilters(true)}><Icon.sliders size={16}/></button>
        </div>
        <CategoryStrip st={st} tweaks={tweaks}/>
        {st.foodTags.length > 0 && <ActiveFoodTags st={st}/>}
        <div style={{ marginTop: 12, marginBottom: 8, fontSize: 12, color: 'var(--muted)' }}>
          {st.filtered.length} {st.filtered.length === 1 ? 'place' : 'places'}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {st.filtered.map(p => <PlaceCard key={p.id} place={p} compact savedSet={savedSet} onToggleSave={onToggleSave}/>)}
        </div>
      </aside>

      <SelectedPopup st={st} savedSet={savedSet} onToggleSave={onToggleSave} offsetForDrawer={drawerOpen}/>
      <MobileSheet st={st} savedSet={savedSet} onToggleSave={onToggleSave}/>
      {st.openFilters && <FilterModal st={st} tweaks={tweaks} onClose={() => st.setOpenFilters(false)}/>}
    </main>
  );
}

function ListHeader({ st, count, city }) {
  return (
    <div style={{ marginBottom: 12 }}>
      <div className="mono" style={{ marginBottom: 4 }}>{city === 'phuket' ? 'Phuket' : 'Bangkok'} · explore</div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10 }}>
        <h2 className="h3" style={{ margin: 0 }}>{count} {count === 1 ? 'place' : 'places'}</h2>
        <button className="btn btn-ghost" style={{ padding: '4px 8px', fontSize: 12 }} onClick={() => st.setOpenFilters(true)}><Icon.sliders size={14}/> Filter</button>
      </div>
      <div style={{ position: 'relative' }}>
        <Icon.search size={16} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--muted)' }}/>
        <input value={st.query} onChange={e => st.setQuery(e.target.value)} placeholder='Search "boat market"…' className="input" style={{ width: '100%', paddingLeft: 36 }}/>
      </div>
    </div>
  );
}

function CategoryStrip({ st, tweaks }) {
  const cats = CATEGORIES.filter(c => !c.optional || tweaks.showCannabis);
  return (
    <div className="hide-scrollbar" style={{ display: 'flex', gap: 6 }}>
      <button onClick={() => st.setCat('all')} className={'chip' + (st.activeCat === 'all' ? ' is-on' : '')}>All</button>
      {cats.map(c => (
        <button key={c.id} onClick={() => st.setCat(c.id === st.activeCat ? 'all' : c.id)}
                className={'chip chip-brand' + (c.id === st.activeCat ? ' is-on' : '')}>
          {c.label}
        </button>
      ))}
    </div>
  );
}

function ActiveFoodTags({ st }) {
  return (
    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8 }}>
      {st.foodTags.map(t => (
        <button key={t} onClick={() => st.setFoodTags(st.foodTags.filter(x => x !== t))} className="chip is-on" style={{ background: 'var(--gold)', color: '#1B1816' }}>
          {t} <span style={{ marginLeft: 4, opacity: .6 }}>×</span>
        </button>
      ))}
    </div>
  );
}

function SelectedPopup({ st, savedSet, onToggleSave, offsetForDrawer }) {
  const place = st.selectedId ? PLACES.find(p => p.id === st.selectedId) : null;
  if (!place) return null;
  return (
    <div className="map-popup only-tablet-up" style={{ right: 24, top: 24, left: offsetForDrawer ? 'calc(min(420px, 86vw) + 24px)' : 'auto' }}>
      <Slot tone={place.slot.tone} label={place.slot.label} sub={place.slot.sub} h={130} style={{ borderRadius: 0 }}/>
      <div style={{ padding: '12px 14px 14px' }}>
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 15, letterSpacing: '-.01em' }}>{place.name}</div>
        <div style={{ fontSize: 11.5, color: 'var(--muted)' }}>{place.subcategory}</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 7, fontSize: 11.5 }}>
          <StarRating value={place.rating}/>
          <span className="dot-sep"/>
          <PriceMark n={place.price}/>
        </div>
        <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
          <Link to={`#/place/${place.id}`} className="btn btn-primary" style={{ flex: 1, justifyContent: 'center', padding: '8px 12px', fontSize: 12 }}>Details</Link>
          <button className="btn btn-sq" onClick={() => onToggleSave(place.id)} aria-label="Save">
            {savedSet.has(place.id) ? <Icon.bookmarkFill size={16}/> : <Icon.bookmark size={16}/>}
          </button>
          <button className="btn btn-sq" onClick={() => st.setSel(null)} aria-label="Close"><Icon.x size={14}/></button>
        </div>
      </div>
    </div>
  );
}

function FabColumn() {
  return (
    <div className="map-fab-col">
      <button className="fab" aria-label="Locate"><Icon.locate size={18}/></button>
      <button className="fab" aria-label="Open in Google Maps"><Icon.ext size={18}/></button>
      <button className="fab" aria-label="Zoom in"><Icon.plus size={16}/></button>
      <button className="fab" aria-label="Zoom out"><Icon.minus size={16}/></button>
    </div>
  );
}

function MobileSheet({ st, savedSet, onToggleSave }) {
  const [open, setOpen] = useState(true);
  return (
    <div className="map-sheet only-mobile" style={{ maxHeight: open ? '50%' : '64px' }}>
      <button onClick={() => setOpen(o => !o)} style={{ background: 'transparent', border: 0, padding: 0 }}>
        <div className="map-sheet__handle"/>
      </button>
      <div className="map-sheet__head">
        <div>
          <span className="h4" style={{ fontSize: 14 }}>{st.filtered.length} places</span>
          {st.activeCat !== 'all' && <span style={{ color: 'var(--muted)', fontSize: 12, marginLeft: 6 }}>· {(CATEGORIES.find(c => c.id === st.activeCat) || {}).label}</span>}
        </div>
        <button className="btn btn-ghost" style={{ padding: 4 }} onClick={() => st.setOpenFilters(true)}><Icon.sliders size={14}/> Filter</button>
      </div>
      <div className="map-sheet__body">
        {st.filtered.map(p => <PlaceCard key={p.id} place={p} compact savedSet={savedSet} onToggleSave={onToggleSave}/>)}
      </div>
    </div>
  );
}

function FilterModal({ st, tweaks, onClose }) {
  const cats = CATEGORIES.filter(c => !c.optional || tweaks.showCannabis);
  return (
    <div className="modal-bg" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 520, padding: 24 }} onClick={e => e.stopPropagation()}>
        <button className="modal__close" onClick={onClose}><Icon.x size={20}/></button>
        <h3 className="h3" style={{ margin: 0 }}>Filter</h3>
        <p className="muted" style={{ fontSize: 13, marginTop: 4, marginBottom: 18 }}>Narrow the {st.filtered.length} results.</p>

        <div className="mono" style={{ marginBottom: 8 }}>Category</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 18 }}>
          <button onClick={() => st.setCat('all')} className={'chip' + (st.activeCat === 'all' ? ' is-on' : '')}>All</button>
          {cats.map(c => (
            <button key={c.id} onClick={() => st.setCat(c.id === st.activeCat ? 'all' : c.id)}
                    className={'chip chip-brand' + (c.id === st.activeCat ? ' is-on' : '')}>{c.label}</button>
          ))}
        </div>

        <div className="mono" style={{ marginBottom: 8 }}>Food filters</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 22 }}>
          {FOOD_FILTERS.map(t => {
            const on = st.foodTags.includes(t);
            return (
              <button key={t}
                onClick={() => st.setFoodTags(on ? st.foodTags.filter(x => x !== t) : [...st.foodTags, t])}
                className={'chip' + (on ? ' is-on' : '')} style={on ? { background: 'var(--gold)', color: '#1B1816' } : null}>
                {t}
              </button>
            );
          })}
        </div>

        <div style={{ display: 'flex', gap: 8, justifyContent: 'space-between' }}>
          <button onClick={() => { st.setCat('all'); st.setFoodTags([]); st.setQuery(''); }} className="btn btn-ghost">Clear all</button>
          <button onClick={onClose} className="btn btn-primary">Show {st.filtered.length} places</button>
        </div>
      </div>
    </div>
  );
}

function EmptyState() {
  return (
    <div className="card card-flat" style={{ textAlign: 'center', padding: 24, color: 'var(--muted)' }}>
      <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 4 }}>No matches.</div>
      <div style={{ fontSize: 12 }}>Try "buffet", "boat market", or "muay thai".</div>
    </div>
  );
}
